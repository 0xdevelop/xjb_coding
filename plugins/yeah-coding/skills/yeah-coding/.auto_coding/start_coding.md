...
⑨ 监听结束信号（满足任意一项即执行紧急存档后提示用户开新会话续做）：
   - 收到 <system-reminder> 类含 compaction / 上下文警告的系统提示
   - 用户消息含"快满了 / token 不足 / 开新会话 / 上下文不够"等明示
   - 当前任务读取文件数已超过约束上限（5 个）
   都未命中 → 直接回到 ①
   ⚠️ 不基于 AI 自估上下文百分比触发；LLM 自感知不准，会过早打断流程
```
...
## 断点续做指引
<!-- 每次任务完成或紧急存档触发时更新，必须足够精确到让全新会话无损恢复 -->
- **当前任务**：TASK-XXX《标题》
...
1. **常规 checkpoint**：每完成一个任务自测全绿后必 commit + push（已在阶段 2 步骤⑥-⑧ 强制），任务边界即天然安全点，无需检测
2. **紧急存档**：在外部信号触发时执行，把 mid-task 进度落盘以便新会话恢复

### 紧急存档触发条件（外部信号，非 AI 自感知）

满足以下**任意一项**即立即执行紧急存档：

- 收到 `<system-reminder>` 类含 compaction / 上下文警告的系统提示
- 用户消息中包含「快满了」「token 不足」「开新会话」「上下文不够」等明示
- 当前任务读取文件数已超过约束上限（5 个）—— 任务规划失误，需要拆任务后重启

> ❌ **不基于 AI 自估上下文百分比触发**。LLM 没有可靠自省机制，估算会失准，过早触发反而频繁打断流程。

# xjb_coding — AI 自驱编码 Skill (Skills-first, MCP-enhanced)
...
**架构原则**：本仓库是宿主中立的 **workflow/plugin 前端**。默认通过 Skills / markdown 模板工作；如果用户显式部署并连接了 [xjb_code](https://github.com/0xdevelop/xjb_code)，就自动优先走 MCP Streamable HTTP 后端。两个项目独立存在，配合使用效果更佳。

```
┌─────────────────┐  trigger word    ┌──────────────────┐  MCP tools     ┌────────────────┐
│ Claude Code /   │ ───────────────▶ │ xjb_coding      │ ─────────────▶ │ xjb_code      │
...
>
> **增强模式**：再部署 `xjb_code` 并在当前宿主配置 MCP，任务状态、锁、远程审批和 dashboard 都由 daemon 承载。
>
...
- `xjb_coding` 不做 telemetry，不上传仓库、diff、prompt、任务状态。
- `xjb_code` 是用户自己部署的通用 MCP 后端，不接 LLM，不把数据转发给模型供应商；SQLite 数据库在你的本机、局域网服务器或私有云里。
...
| 工具 | 前提 |
|------|------|
| Claude Code | Claude Code 已装（`claude --version` 能跑）+ GitHub SSH key 已配好（`ssh -T git@github.com` 能成功）+ 已加入本仓库和 xjb_code 仓库 collaborator |
| Codex | Codex CLI 已装（`codex --version` 能跑）+ GitHub SSH key 已配好（`ssh -T git@github.com` 能成功）+ 已加入本仓库和 xjb_code 仓库 collaborator |
| Go | ≥ 1.25.0（编译 xjb_code daemon 用） |
...
## xjb_code daemon 安装（可选增强，所有宿主通用）

不装 daemon 也能用：skill 会自动回退到 markdown 模式。装上 daemon 后，任务状态、并发锁、断点续做、远程审批、Web dashboard、动态 skills 路由都由 `xjb_code` 承载（WeChat 控制面待回归）。

`xjb_code` 也可以脱离 `xjb_coding` 单独使用：任何支持 MCP Streamable HTTP 的客户端都能直接连 `http://<host>:12100/` 调用工具（MCP 挂根路径）。
...
cd xjb_code
go build -o xjb_code .
./xjb_code &                           # 监听 :12095(JSON-RPC) :12100(MCP) :12102(WS) :12103(gRPC)
...
`./xjb_code` 默认在前台运行；要装成开机自启的 systemd / brew service 看 xjb_code 仓库 README。

### 局域网 / 云部署

参见 [xjb_code README](https://github.com/0xdevelop/xjb_code#部署模式) — 推荐 daemon 自身只跑 plain HTTP，前面挂 nginx/Caddy 卸载 TLS + Bearer token 鉴权。
...
> 方法清单与入参 schema 的**唯一事实源**是 xjb_code 仓的 `docs/api_methods.md`（`gen_api_docs.sh` 生成）。本仓不复制 schema，只讲工作流用法。

---

## Claude Code Plugin 安装

Claude Code 是一个宿主前端。安装 plugin 后先使用 Skills 模式；如果你已配置 `xjb_code` MCP，skill 会自动优先调用 MCP 工具。
...
> **多账号 / 私库 SSH host alias**：如果你机器上是多 GitHub 账号且为本仓库配了独立 alias（如 `~/.ssh/config` 里 `Host github-0xdevelop`），把第一行换成：
>
> ```
> /plugin marketplace add git@github-0xdevelop:0xdevelop/xjb_coding.git
> ```

### 2. MCP 自动注册（v0.0.16 起）

plugin 自带 `.mcp.json`：安装后自动注册 `xjb-code` MCP 服务（`http://localhost:12100/`），`/plugin` 管理器里可直接看到该服务的连接状态——本机跑着 daemon 就是零配置直连，没跑 daemon 则显示未连接（无害，skill 自动走 markdown 回退）。
...
输入 `/plugin` 打开管理器，切到 **Installed** 标签页应看到 `xjb-coding`，状态为已加载；MCP 服务列表应有 `xjb-code`。或直接发触发词：
...
| `install` 后 AI 不识别 `使用 xjb_coding` | 跑 `/reload-plugins`；仍不行就重启 Claude Code 会话 |
| 用 GitHub HTTPS shorthand `0xdevelop/xjb_coding` 装失败 | 私库走 https 鉴权链很容易出问题；改用完整 SSH URL `git@github.com:0xdevelop/xjb_coding.git` |
...
Codex 侧不把 MCP server 硬编码进 plugin manifest。安装 plugin 得到默认 Skills 能力；你用 `codex mcp add` 显式配置 `xjb_code` 后，工作流自动切到 MCP 增强模式。
...
>
> ```bash
> codex plugin marketplace add git@github-0xdevelop:0xdevelop/xjb_coding.git --ref latest
> ```
...
应看到 `xjb-coding-codex-marketplace`，且 `xjb-coding@xjb-coding-codex-marketplace` 为 installed / enabled。
...
Codex 适配入口会优先使用 `xjb-coding-codex` skill，把 Codex 作为架构 controller，worker 只做小范围实现。
...
```bash
codex plugin marketplace add /path/to/xjb_coding
codex plugin add xjb-coding@xjb-coding-codex-marketplace
```
...
git clone git@github.com:0xdevelop/xjb_coding.git
cp -r xjb_coding/skills/xjb-coding ~/.hermes/skills/
...
装好后在 Hermes 会话发触发词 `使用 xjb_coding` 即进入 INIT 流程；`xjb_code` daemon 检测与 MCP 增强逻辑与其他宿主一致。

---

## OpenClaw 安装

OpenClaw 遵循 [AgentSkills](https://agentskills.io) 规范（与 hermes 同标准），本仓 skill 目录结构与 frontmatter 天然兼容（`skills/xjb-coding/` 根即 SKILL.md）：

```bash
# clone 后从本地目录安装（装到当前 workspace 的 skills/）
git clone git@github.com:0xdevelop/xjb_coding.git
openclaw skills install ./xjb_coding/skills/xjb-coding

# 或装成全局（~/.openclaw/skills，所有本机 agent 可见）
openclaw skills install ./xjb_coding/skills/xjb-coding --global
```

装好后发触发词 `使用 xjb_coding` 即进入 INIT 流程；MCP 客户端能力随 OpenClaw 自身配置，未连 daemon 时照常走 markdown 回退。
...
```bash
cd ~/Code/myapp
claude   # 或 codex / 其他已安装 xjb_coding 的宿主
```
...
```
使用 xjb_coding
初始化 xjb_coding /path/to/your/project
setup xjb_coding
安装 xjb_coding
```
...
`xjb_coding` 默认是规则/上下文层；如果需要让 Codex controller 通过终端调起 Claude Code worker，可以使用仓库内脚本：
...
> 已经在用的项目里 `.auto_coding/` **不会被自动覆盖**——它是你项目的本地工作产物。要升级模板，在该项目重发 `初始化 xjb_coding`，AI 会问"已有 .auto_coding/，是否覆盖？(y/n)"。
...
**Claude Code**：
```
/plugin uninstall xjb-coding@xjb-coding-marketplace
/plugin marketplace remove xjb-coding-marketplace
```

**Codex**：
```bash
codex plugin remove xjb-coding@xjb-coding-codex-marketplace
codex plugin marketplace remove xjb-coding-codex-marketplace
```
...
```
读 skills/xjb-coding/SKILL.md
```

发触发词 `使用 xjb_coding`，INIT 流程会自动检测工具类型并部署对应配置：
...
**Q：必须装 xjb_code MCP daemon 才能自驱吗？**
A：不是。自驱动力来自宿主执行循环——会话内主循环、宿主定时循环、cron、headless CLI（`claude -p` / `codex exec`）、外部编排器，任何能反复触发 `--tick` 的机制都行。需求 / 任务 / 锁 / 审批 / 会话是同一套工作流语义，markdown 文件与 xjb_code DB 只是它的两个后端实现。MCP 后端换来的是：强一致 DB 行锁、跨机器状态、远程审批（web/手机）、实时观测。单机单 agent 场景 markdown 后端就够。
...
A：在该项目里重发 `初始化 xjb_coding`，AI 会问"已有 .auto_coding/，是否覆盖？(y/n)"。回 y 即升级。注意：`requirements/`、`tasks/TRACKER.md` 等本地工作产物会被覆盖，谨慎。

**Q：可以同时在多个项目用吗？**
A：可以。plugin 装一次，每个项目独立 INIT。每个项目的 `.auto_coding/` 是独立工作目录，互不影响。

**Q：和 CLAUDE.md 是什么关系？**
A：plugin 把模板 `CLAUDE.md` 复制到你项目根，作为 Claude Code 的项目级指令；它只告诉 Claude Code"本项目用 xjb_coding，触发词是…"。真正的执行规范在 `.auto_coding/start_coding.md` 里。
...
A：MCP 模式下靠 `xjb_code` 的 DB 行级锁（`task.lock`）；markdown fallback 模式才使用 `.auto_coding/tasks/DISPATCH.md` 文件锁，适合单机兜底，不建议多人长期并发。

**Q：能不能不通过 AI 触发，纯手工部署？**
A：能。`cp -r skills/xjb-coding/.auto_coding/ /your/project/root/`，然后在该项目让任意 AI 工具发触发词 `读 .auto_coding/start_coding.md` 进入编码流程。
...
│   ├── xjb-coding/                      ← 主 skill：所有模板文件单一存放点
...
│   │       └── tasks/TRACKER.md
│   └── xjb-coding-codex/                ← Codex controller/worker 适配入口
│       └── SKILL.md
